...
c
c
c Remarks: Adapted from io_write_frame_r_hjs.f
c Authors:  Herve Beust 
c Date:    Jun 9, 2008; revised Jun 19

      subroutine io_write_frame_r_hjs_tid(time,nbod,tid,ntp,oloc,matp,
     &           umat,mass,eta,mu,xj,yj,zj,vxj,vyj,vzj,rotx,roty,rotz,
     &           etatp,xjt,yjt,zjt,vxjt,vyjt,vzjt,istat,oname,
     &           iu,fopenstat)
...
c...  Internals
      ...
      real*8 gm,msys,bot,ci,co,si,so,rot(2),irot(2),orot(2)
...
c...  write out planet orbits
     ...
     &          ialpha,a,e,inc,capom,omega,capm)
         ...
         if (tid(i).ne.0) then
           ...
           k = 1
           do while (oloc(i,k).ne.1)
             k = k + 1
           end do
           j = 1
           do while (oloc(i,j).ne.-1)
             j = j + 1
           end do

           rrot = sqrt(rotx(j)*rotx(j) + roty(j)*roty(j) 
     &                                           + rotz(j)*rotz(j))
           rot(1) = rrot/n
           irot(1) = acos((si*(rotx(j)*so-roty(j)*co)+ci*rotz(j))/rrot)
           orot(1) = atan2(co*rotx(j)+so*roty(j),
     &              ci*(so*rotx(j)-co*roty(j))+si*rotz(j)) 
           rrot = sqrt(rotx(k)*rotx(k) + roty(k)*roty(k) 
     &                                           + rotz(k)*rotz(k))
           rot(2) = rrot/n
           irot(2) = acos((si*(rotx(k)*so-roty(k)*co)+ci*rotz(k))/rrot)
           orot(2) = atan2(co*rotx(k)+so*roty(k),
     &              ci*(so*rotx(k)-co*roty(k))+si*rotz(k)) 
c           print*,'irot',irot(1)*180./PI,irot(2)*180./PI
           print*,'rot/n',sngl(rot(1:2)),'irot',
