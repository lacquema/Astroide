...
c Last revision: 12/27/96
	...
	real*8 vxh(NPLMAX),vyh(NPLMAX),vzh(NPLMAX)
        ...
        real*8 LostEnergy,LostH

	integer istat(NTPMAX,NSTAT),i1st
	integer nbod,ntp,nleft,i,j,indice
...
c Initialize initial time and times for first output and first dump
	...
	tdump = t0 + dtdump
        ...
        call io_init_mass_loss(t,indice,TabulatedTime,TabulatedMass)
        ...
        masst = diskMass / ntp *3.7693444538248667d-02 
...
c        call anal_energy_tp(nbod,mass,istat,j2rp2,j4rp4,xh,yh,zh,
c     &           vxh,vyh,vzh,ntp,masst,xht,yht,zht,vxht,vyht,vzht,ke,
c     &           pot,energy,eltot)   
c        energylast = energy
c        Hlast = sqrt(eltot(1)**2+eltot(2)**2+eltot(3)**2)
c        Envarlast = 0.d0
c        Hvarlast =  0.d0 
...
c***************here's the big loop *************************************
     ...
     &       ((ntp.eq.0).or.(nleft.gt.0)) )
          ...
          call rmvs3_mass_loss(t,indice,nbod,mass,
     &                           TabulatedTime,TabulatedMass)


 	     call rmvs3_step_mig(i1st,t,nbod,ntp,mass,j2rp2,j4rp4,
     &         xh,yh,zh,vxh,vyh,vzh,xht,yht,zht,vxht,vyht,
     &         vzht,istat,rstat,dt,masst,LastEnergy,LastH,
     &         LostEnergy,LostH)
...
c             tfrac = (t-t0)/(tstop-t0)
c             write(*,998) t,tfrac,nleft
c 998         format(' Time = ',1p1e12.5,': fraction done = ',0pf5.3,
c     &            ': Number of active tp =',i4)
c	     call io_dump_pl(trim(diro)//'/'//'dump_pl.dat',nbod,mass,xh,yh,zh,
c     &                 vxh,vyh,vzh,lclose,iflgchk,rplsq,j2rp2,j4rp4)
c	     call io_dump_tp(trim(diro)//'/'//'dump_tp.dat',ntp,xht,yht,zht,
c     &                      vxht,vyht,vzht,istat,rstat)
c	     call io_dump_param('dump_param.dat',t,tstop,dt,dtout
c     &           ,dtdump,iflgchk,rmin,rmax,rmaxu,qmin,lclose,outfile)
