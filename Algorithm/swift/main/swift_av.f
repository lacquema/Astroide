...
c Copy initial files input into work directory
     ...
     &       .or.(fopenstat(1:3).eq.'NEW')) then
          dataname = 'cp go_av.sh '//trim(diro)
...
c...  Internals:
      ...
      real*8 a,ap,e,q,gm,gmp,mu,f2mi
     ...
     &           cl = 2.9979248d8*86400.d0*365.2422d0/1.4959787061d11
      ...
      real*8 dvectpeg(3,imax),dvectphg(3,imax)
      real*8 dvecteg(3,imax),dvecthg(3,imax)
...
c... On compte d'abord les effets de marée
      ...
      do i=1,2
        f2 = ff(i)
        f2mi = ff(i+8)
        f3 = (rx(i)*rx(i)+ry(i)*ry(i)-2.d0*rz(i)*rz(i))
        f2f3 = f2*f3
        f6 = f2*exq3/(q3*ep3)

        gqdx(i) = 0.d0
        gqdy(i) = -0.5d0*f2f3*f11
        gqdz(i) = f2*ry(i)*rz(i)*f11
        gqdy(i) = gqdy(i) + f2mi*hh1*exq3*e*f5
        ...
        gtfx(i) = -f2*f9
        gtfx(i) = gtfx(i) + 11.d0*rz(i)*f2*f8
        gtfz(i) = -rx(i)*f2*f8
        gtfy(i) = 0.d0

        dtfz(i) = -f10*f2
        dtfx(i) = f2*rx(i)*hh3*f7
        dtfy(i) = f2*ry(i)*hh5*f7
        dtfz(i) = dtfz(i) + f2*rz(i)*hh6*f7
...
c........ Maintenant les perturbations planétaires

      do i=2,imax
        select case (i)
          case(2)
            call ordre2(mass,ap,gm,gmp,nn,nnp,alpha2,alpha3,e,ex2,
     &         exq,exq2,excp,exqp3,exqp4,ff(5),z,dhdtx(2),dhdty(2),
     &         dhdtz(2),drundtx(2),drundty(2),drundtz(2),
     &         dhpdtx(2),dhpdty(2),dhpdtz(2),
     &         drunpdtx(2),drunpdty(2),drunpdtz(2))
          case(3)
            call ordre3(mass,ap,gm,gmp,nn,nnp,alpha3,alpha4,e,ex2,
     &         exq,exq2,excp,excp2,exqp4,exqp5,exqp6,z,dhdtx(3),
     &         dhdty(3),dhdtz(3),drundtx(3),
     &         drundty(3),drundtz(3),dhpdtx(3),
     &         dhpdty(3),dhpdtz(3),drunpdtx(3),drunpdty(3),drunpdtz(3))
          case(4)
            call ordre4(mass,ap,gm,gmp,nn,nnp,alpha4,alpha5,e,ex2,
     &         exq,exq2,exq4,excp,excp2,exqp2,exqp6,exqp7,exqp8,
     &         z,dhdtx(4),dhdty(4),dhdtz(4),drundtx(4),drundty(4),
     &         drundtz(4),dhpdtx(4),dhpdty(4),dhpdtz(4),drunpdtx(4),
     &         drunpdty(4),drunpdtz(4))
          case(5)       
            call ordre5(mass,ap,gm,gmp,nn,nnp,alpha4,alpha5,e,ex2,
     &         exq,exq2,exq4,excp,excp2,exqp2,exqp6,exqp7,exqp8,z,
     &         dhdtx(5),dhdty(5),dhdtz(5),drundtx(5),drundty(5),
     &         drundtz(5),dhpdtx(5),dhpdty(5),dhpdtz(5),drunpdtx(5),
     &         drunpdty(5),drunpdtz(5))
        end select
        dvecthg(1,i) = dhdtx(i)*e1(1) + dhdty(i)*e2(1) + dhdtz(i)*e3(1) 
        dvecthg(2,i) = dhdtx(i)*e1(2) + dhdty(i)*e2(2) + dhdtz(i)*e3(2) 
        dvecthg(3,i) = dhdtx(i)*e1(3) + dhdty(i)*e2(3)+ dhdtz(i)*e3(3) 
        dvecteg(1,i) = drundtx(i)*e1(1) + drundty(i)*e2(1)
     &                                         + drundtz(i)*e3(1) 
        dvecteg(2,i) = drundtx(i)*e1(2) + drundty(i)*e2(2)
     &                                         + drundtz(i)*e3(2) 
        dvecteg(3,i) = drundtx(i)*e1(3) + drundty(i)*e2(3)
     &                                         + drundtz(i)*e3(3) 
        dvectphg(1,i) = dhpdtx(i)*e1p(1) + dhpdty(i)*e2p(1) 
     &                                         + dhpdtz(i)*e3p(1) 
        dvectphg(2,i) = dhpdtx(i)*e1p(2) + dhpdty(i)*e2p(2)
     &                                         + dhpdtz(i)*e3p(2) 
        dvectphg(3,i) = dhpdtx(i)*e1p(3) + dhpdty(i)*e2p(3)
     &                                         + dhpdtz(i)*e3p(3) 
        dvectpeg(1,i) = drunpdtx(i)*e1p(1) 
     &                      + drunpdty(i)*e2p(1) + drunpdtz(i)*e3p(1) 
        dvectpeg(2,i) = drunpdtx(i)*e1p(2)
     &                      + drunpdty(i)*e2p(2) + drunpdtz(i)*e3p(2) 
        dvectpeg(3,i) = drunpdtx(i)*e1p(3)
...
c........
      ...
      do i = 2,imax
        dvecth(1) = dvecth(1) + dvecthg(1,i) 
        dvecth(2) = dvecth(2) + dvecthg(2,i)
        dvecth(3) = dvecth(3) + dvecthg(3,i)
        dvecte(1) = dvecte(1) + dvecteg(1,i)
        dvecte(2) = dvecte(2) + dvecteg(2,i)
        dvecte(3) = dvecte(3) + dvecteg(3,i)
        dvectph(1) = dvectph(1) + dvectphg(1,i) 
        dvectph(2) = dvectph(2) + dvectphg(2,i)
        dvectph(3) = dvectph(3) + dvectphg(3,i)
        dvectpe(1) = dvectpe(1) + dvectpeg(1,i)
        dvectpe(2) = dvectpe(2) + dvectpeg(2,i)
        dvectpe(3) = dvectpe(3) + dvectpeg(3,i)
      end do
...
c     & sngl(2.d0/h/h*(vecth(1)*dvecth(1)+vecth(2)*dvecth(2)c     &                                +vecth(3)*dvecth(3))),
...
C**************************************************************************
      ...
      subroutine ordre2(mass,ap,gm,gmp,nn,nnp,alpha2,alpha3,e,ex2,
     &         exq,exq2,excp,exqp3,exqp4,ff5,z,dhdtx,dhdty,dhdtz,
     &         drundtx,drundty,drundtz,dhpdtx,dhpdty,dhpdtz,
     &         drunpdtx,drunpdty,drunpdtz)
       
      include 'swift.inc'

c...  Inputs: 
      real*8 mass(3)
      real*8 ap,gm,gmp,nn,nnp,alpha2,alpha3
      real*8 e,ex2,exq,exq2,excp,exqp3,ff5,exqp4
      real*8 z(3,3)
...
c...  Internals:
      ...
      z332 = z(3,3)*z(3,3)
      z232 = z(2,3)*z(2,3)
      dhdtx = -0.75d0*dd*z(3,3)*z(2,3)*exq2
      dhdty = +0.75d0*dd*z(1,3)*z(3,3)*(4.d0*ex2+1.d0)
      dhdtz = -3.75d0*dd*z(1,3)*z(2,3)*ex2

      dd = mass(3)*alpha3*nn*e*exq/exqp3/gm
      drundtx = 3.75d0*dd*z(2,3)*z(1,3)
      drundty = 0.75d0*dd*(-3.d0+5.d0*z232+4.d0*z332)
      drundtz = +0.75d0*dd*z(3,3)*z(2,3)

      dd = gmp*ff5*alpha2/ap/exqp3
      dhpdtx = -0.75d0*dd*(z(3,3)*z(3,2)+
     &          (5.d0*z(2,3)*z(2,2)+4.d0*z(3,3)*z(3,2))*ex2)
      dhpdty = 0.75d0*dd*(z(3,3)*z(3,1)+
     &          (5.d0*z(2,3)*z(2,1)+4.d0*z(3,3)*z(3,1))*ex2)
      dhpdtz = 0.d0

      dd = ff5*alpha2*nnp*excp/exqp4
      drunpdtx = 0.d0
      drunpdty = 0.375d0*dd*(-1.d0+3.d0*z332+
     &          (-9.d0+15.d0*z232+12.d0*z332)*ex2)
      drunpdtz = 0.75d0*dd*(z(3,3)*z(3,2)+
     &          (5.d0*z(2,3)*z(2,2)+4.d0*z(3,3)*z(3,2))*ex2)
