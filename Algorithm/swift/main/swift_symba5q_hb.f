...
c Date:    11/21/96
c Last revision: 05/02/04 H. Beust (directories)
...
c Get data for the run and the test particles
     ...
     &     iflgchk,rmin,rmax,rmaxu,qmin,lclose,outfile,diro,dirs,
     &     gname,fopenstat)
...
c Prompt and read name of planet data file
 ...
 3    write(*,*) 'Enter the smallest mass to self gravitate :'
      read(*,*) mtiny
      write(*,*) ' mtiny = ',mtiny
...
c...  must initize discard io routine
      if(btest(iflgchk,4))  then ! bit 4 is set
         call io_discard_mass_hb(0,t,0,mass(1),rpl(1),xh(1),yh(1),
     &        zh(1),vxh(1),vyh(1),vzh(1),iud,-1,diro,fopenstat)
      endif
...
c***************here's the big loop *************************************
      ...
      do while ( (t .le. tstop) .and. (nbod.gt.1) )
         ...
         call symba5q_step_pl(i1st,t,nbod,nbodm,mass,j2rp2,j4rp4,
