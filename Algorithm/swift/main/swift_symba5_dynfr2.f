...
c...  Internals:
      ...
      integer ialpha,ie,j
      real*8 a,e,inc,capom,omega,capm,peri,rj
      real*8 s,b,bmax,dU,U,bhill,Vesc2,fr
      PARAMETER (bmax=6.d0) ! maximal multiple of a Hill sphere used for drag 
...
c compute the unperturbed heliocentric orbit of the body i when 
c on the target plane 
                  xr=xr+xh(j)
                  yr=yr+yh(j)
                  zr=zr+zh(j)
                  call orbel_xv2el(xr,yr,zr,vxr,vyr,vzr,gm,
     &                 ialpha,a,e,inc,capom,omega,capm)
                  bhill=bmax*rhill(j)
                  rj=sqrt(xh(j)**2+yh(j)**2+zh(j)**2)
c     compute the drag if the impact parameter is in the Hill sphere
c     or if it is in the extended Hill sphere but a collision would be possible
c     by an appropriate choice of Omega and omega
                  if(b.lt.rhill(j).or.
     $                 (b.lt.bhill.and.a*(1.d0-e).le.rj
     $                 .and.a*(1.d0+e).ge.rj.and.rj*sin(inc).ge.zh(j))
     $                 )then
