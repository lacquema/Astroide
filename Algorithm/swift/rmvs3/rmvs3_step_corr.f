...
c Last revision: 
      ...
      subroutine rmvs3_step_corr(nsta,i1st,time,nbod,ntp,mass,
...
c...  Internals
      integer i1sttp,i1stpl,i1sto,icflg,i,j,np,insta,s
...
c... If nsta>1 first do a positive (s=1) correction step
      ...
      if (nsta.gt.1) then
        s = 1
        call  step_dkd_corr(nsta,s,i1st,time,nbod,ntp,mass,
     &       j2rp2,j4rp4,xh,yh,zh,vxh,vyh,vzh,xht,yht,zht,
     &       vxht,vyht,vzht,istat,rstat,dt)
      end if
...
c...  step
c      write(*,*)'starting nstaloop'
      nstaloop : do insta = 1,nsta
        i1sttp = i1st
        i1sto = i1st
...
c.... if not just do a normal step and cycle the loop
        if (icflg.eq.0) then
           call step_dkd(i1st,time,nbod,ntp,mass,j2rp2,j4rp4,xh,yh,zh,
...
c...  ENCOUNTER STUFF FROM HERE ON!!!!!
        ...
        i1st = 0
        i1sttp = i1st  ! We need to recalculate accel arrays
...
c... do a full step for the planets
        i1stpl = i1st
        call step_dkd_pl(i1stpl,nbod,mass,j2rp2,j4rp4,xh,yh,zh,
...
c...  do a full step
        ...
        call step_dkd_tp(i1sto,nbod,ntp,mass,j2rp2,j4rp4,
     &         xhm,yhm,zhm,xht,yht,zht,vxht,vyht,vzht,istattmp,dt)	
...
c...  If nsta>1 first do a negative (s=-1) correction step
