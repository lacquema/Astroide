...
c
      ...
      subroutine rmvs3_step_mig_fix(i1st,time,nbod,ntp,mass,j2rp2,j4rp4,
...
c.... if not just do a normal step and leave
      if(icflg.eq.0) then
         call step_kdk_mig_fix(i1st,time,nbod,ntp,mass,j2rp2,j4rp4,
     &        xh,yh,zh,
...
c... do a full step for the planets
      i1stpl = i1st
      call step_kdk_mig_fix_pl(i1stpl,nbod,mass,j2rp2,j4rp4,xh,yh,zh,
     &     vxh,vyh,vzh,dt,avar)
