...
c Last revision: 5/13/99
      ...
      recursive subroutine symbatr_step_recur(t,nbod,nbodm,mass,
     &     ireci,ilevl,iecnt,ielev,rhill,xh,yh,zh,vxb,vyb,vzb,
     &     lclose,rpl,mergelst,mergecnt,dt0,eoff,svdotr,ielc,ielst)
...
c...     Do we need to go deeper?
         ...
         do ie=1,ielc
            ...
            if((ielev(i).ge.ireci).and.(ielev(j).ge.ireci)) then
               call symbatr_chk(rhill,nbod,i,j,mass,xh,yh,zh,
     &              vxb,vyb,vzb,dtl,irecp,ieflg,svdotr(ie))
         ...
         sgn = 1.0d0
         call symbatr_kick(nbod,mass,irecp,iecnt,ielev,
     &        rhill,xh,yh,zh,vxb,vyb,vzb,dth,sgn,ielc,
     &        ielst)

         call symbatr_helio_drift(nbod,ielev,ireci,mass,xh,
     &        yh,zh,vxb,vyb,vzb,dtl)
         if(icflg.ne.0) then
            call symbatr_step_recur(t,nbod,nbodm,mass,irecp,ilevl,
     ...
     &        rhill,xh,yh,zh,vxb,vyb,vzb,dth,sgn,ielc,ielst)
         ...
         if( lclose ) then       ! look for mergers
            do ie=1,ielc
               ...
               j = ielst(2,ie)
               if((ielev(i).ge.ireci).and.(ielev(j).ge.ireci)) then
                  call symbatr_merge(t,dtl,nbod,i,j,mass,xh,yh,zh,
...
c...        Do we need to go deeper?
            ...
            do ie=1,ielc
               ...
               j = ielst(2,ie)
               if((ielev(i).ge.ireci).and.(ielev(j).ge.ireci)) then
                  call symbatr_chk(rhill,nbod,i,j,mass,xh,yh,
