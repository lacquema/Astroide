...
c                                      (logical*2 scalar)
c                 diro          ==> Directory (character*(chlen))
...
c Last revision: 05/03/04 H. Beust (directory information)
      ...
      subroutine discard_massive5_hb(time,dt,nbod,mass,xh,yh,zh,
     &     vxh,vyh,vzh,rmin,rmax,rmaxu,qmin,lclose,diro,rpl,
...
c...  check perihelion distance
     ...
     &           vxh,vyh,vzh,ke,pot,ei,eltot)
            ...
            call io_discard_mass_hb(1,time,i,mass(i),rpl(i),xh(i),yh(i),
     &           zh(i),vxh(i),vyh(i),vzh(i),iu,iwhy(i),diro,cdummy)
            do j=i,nbod-1
               iwhy(j) = iwhy(j+1)
            enddo
            i1st = 0
