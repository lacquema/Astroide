...
c G = 1., and we allow diff. masses.  Also returns square of total ang. mom.
      ...
      subroutine anal_energy_tp(nbod,mass,istat,j2rp2,j4rp4,xh,yh,zh,
     &           vxh,vyh,vzh,ntp,masst,xht,yht,zht,vxht,vyht,vzht,ke
     &           ,pot,energy,eltot,Etheo,Htheo)
...
c...  Output
      real*8 energy,eltot(3),ke,pot,Etheo,Htheo
...
c...  Executable code 
     ...
     &                  xbt,ybt,zbt,vxbt,vybt,vzbt)
      ...
      if(istat(i,1).eq.0) then 
          call orbel_xv2el(xht(2),yht(2),zht(2),vxht(2),vyht(2),vzht(2),
     &    mass(1),ialpha,a,e,inc,capom,omega,capm)

         Etheo=Etheo - mass(1)*masst/(2*a)
         Htheo=Htheo + sqrt(mass(1)*a*(1-e**2))*masst
