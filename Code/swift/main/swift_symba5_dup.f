...
c         Particle multipplication when getting close enough to planet.
c Authors:  H. Beust & J. Vandeportal
c Date:    Sep. 16, 2009
      ...
      real*8 rmin,rmax,rmaxu,qmin,mtiny,msmall,mdisk
      logical*2 lclose 
      integer isenc,ihills,ierr
      integer mergelst(2,NTPMAX),mergecnt
      integer*2 iecnt(NTPMAX)

      character*(chlen) outfile,inparfile,inplfile,fopenstat,
     &               diro,dirs,gname,dataname,genfile,massfile
...
c Prompt and read name of planet data file
 ...
 999  format(a)
     ...
     &     xh,yh,zh,vxh,vyh,vzh,rpl,rhill,j2rp2,j4rp4)
      ...
      read(*,999) massfile
      call io_open(7,massfile,'old','formatted',ierr)
      read(7,*)iouter
      read(7,*)mtiny
      read(7,*)msmall
      read(7,*)mdisk
...
c Copy initial files input into work directory
     ...
     &       .or.(fopenstat(1:3).eq.'NEW')) then
          ...
          call system(dataname)
          dataname = 'cp '//trim(massfile)//' '//trim(diro)
          call system(dataname)
      end if
...
c***************here's the big loop *************************************
      ...
      do while ( (t .le. tstop) .and. (nbod.gt.1) )
         ...
         call symba5_step_pl_par(i1st,t,nbod,nbodm,mass,j2rp2,j4rp4,
     &        xh,yh,zh,vxh,vyh,vzh,dt,lclose,rpl,isenc,
     &        mergelst,mergecnt,iecnt,eoff,rhill,mtiny)

         t = t + dt

         call symba5_duplicate(nbod,nbodm,iouter,msmall,mtiny,
