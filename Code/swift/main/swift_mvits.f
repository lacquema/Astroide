...
c		      SWIFT_MVITS.F
c**********************************************************************
c Uses the individual timestep MVS integrator
c
...
c                   planet time step file     dti.in
c
...
c Last revision: 12/27/96
	...
	character*(chlen) outfile,inparfile,inplfile,
     &                            intpfile,fopenstat
...
c***************here's the big loop *************************************
     ...
     &       ((ntp.eq.0).or.(nleft.gt.0)) )
 	     ...
 	     call mvits_step(i1st,t,nbod,ntp,mass,j2rp2,j4rp4,
