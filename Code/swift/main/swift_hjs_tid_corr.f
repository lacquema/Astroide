...
c                   AND ADDS SYMPLECTIC CORRECTION 
c                    Wisdom 2006, AJ 131, 2294
c
...
c Date:    Mar 10, 2010
c Remark : based on swift_hjs_tid.f
	...
	real*8 rmin,rmax,rmaxu,qmin,rplsq(NPLMAX),krpl5(NPLMAX)
        real*8 Qlov(NPLMAX)
        integer tid(NPLMAX)
        logical*2 lclose 
...
c Get data for the run and the test particles
	...
	read(*,999) inparfile
	call io_init_param_corr(inparfile,t0,tstop,dt,dtout,dtdump,
     &         iflgchk,nsta,rmin,rmax,rmaxu,qmin,lclose,diro,dirs,
...
c Prompt and read name of massive bodies data file
	...
	read(*,999) inplfile
999 	format(a)
	call io_init_pl_hjs_tid(inplfile,lclose,iflgchk,nbod,tid,oloc,
     &       mass,eta,mu,mat,umat,xj,yj,zj,vxj,vyj,vzj,rotx,roty,rotz,
     &       inert,krpl5,Qlov,rplsq)
...
c...    Do the initial io write
     ...
     &           mass,eta,mu,xj,yj,zj,vxj,vyj,vzj,rotx,roty,rotz,
     &           etatp,xjt,yjt,zjt,vxjt,vyjt,vzjt,istat,
     &           trim(diro)//'/'//outfile,iub,fopenstat)
        endif
        if(btest(iflgchk,1))  then ! bit 1 is set
           call io_write_frame_r_hjs_tid(t0,nbod,tid,ntp,oloc,matp,umat,
     &           mass,eta,mu,xj,yj,zj,vxj,vyj,vzj,rotx,roty,rotz,
     &           etatp,xjt,yjt,zjt,vxjt,vyjt,vzjt,istat,
     &           trim(diro)//'/'//outfile,iub,fopenstat)
        endif
...
c     &       ((ntp.eq.0).or.(nleft.gt.0)) )
          do while (ok)
            ...
            call step_dkd_hjs_tid(nsta,i1st,t,nbod,tid,ntp,
...
c if it is time, output orb. elements, 
	  if (t .ge. tout) then 
            if (btest(iflgchk,0))  then    ! bit 0 is set
              call io_write_frame_hjs(t,nbod,tid,ntp,oloc,matp,umat,
     &           mass,eta,mu,xj,yj,zj,vxj,vyj,vzj,etatp,xjt,yjt,zjt,
     &           vxjt,vyjt,vzjt,istat,trim(diro)//'/'//outfile,
     &           iub,fopenstat)
            endif
            if (btest(iflgchk,1))  then ! bit 1 is set
              call io_write_frame_r_hjs_tid(t,nbod,tid,ntp,oloc,
     &           matp,umat,mass,eta,mu,xj,yj,zj,vxj,vyj,vzj,
     &           rotx,roty,rotz,etatp,xjt,yjt,zjt,vxjt,vyjt,vzjt,
     &           istat,trim(diro)//'/'//outfile,iub,fopenstat)
            endif
...
c If it is time, do a dump
 ...
 998         format(' Time = ',1p1e12.5,': fraction done = ',0pf5.3,
     &            ': Number of active tp =',i4)
             call io_dump_pl_hjs_tid(trim(diro)//'/'//'dump_pl.dat',
     &                    nbod,tid,oloc,mass,umat,
     &                    xj,yj,zj,vxj,vyj,vzj,rotx,roty,rotz,
     &                    inert,krpl5,Qlov,lclose,iflgchk,rplsq)
...
c Do a final dump for possible resumption later 
        ...
        call io_dump_pl_hjs_tid(trim(diro)//'/'//'dump_pl.dat',
     &                 nbod,tid,oloc,mass,umat,
     &                 xj,yj,zj,vxj,vyj,vzj,rotx,roty,rotz,inert,
