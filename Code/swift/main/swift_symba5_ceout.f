...
c--   compute r cdot v for each couple of encountering massive bodies
      do ie=1,ielc
         i = ielst(1,ie)
         j = ielst(2,ie)
...
c     orbital elements. TBD
            gm=mass(i)+mass(j)
            call orbel_xv2el(xr,yr,zr,vxr,vyr,vzr,gm,
     &           ialpha,a,e,inc,capom,omega,capm)
            if(ialpha.gt.0)then
               peri=-a*(1.d0-e)
            elseif(ialpha.lt.0)then
               peri=a*(1.d0-e)
            else
               peri=a
            endif
            if(peri.le.rhill(i).or.peri.le.rhill(j))then
c              if( (mass(i).ge.mtiny) .and. (mass(j).ge.mtiny))then
              if( mass(j).lt.mtiny )then
                  open(unit=90,file='ce.out',
     %                 status='unknown',position='append')
                  write(90,*)time
                  write(90,*)i,mass(i)
                  write(90,*)xh(i),yh(i),zh(i)
                  write(90,*)vxh(i),vyh(i),vzh(i)
                  write(90,*)j,mass(j)
                  write(90,*)xh(j),yh(j),zh(j)
                  write(90,*)vxh(j),vyh(j),vzh(j)
                  close(90)
c               else
c                  open(unit=91,file='ce.list',
