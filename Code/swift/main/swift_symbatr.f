...
c Last revision: 12/27/96
      ...
      include 'swift.inc'
      include '../symbatr/symbatr.inc'
      include '../symbatr/inc/treedefs.f'
...
c Get data for the run and the test particles
      write(*,*) 'Enter name of SYMBA parameter data file : '
      read(*,999) inparfile
      call io_init_param(inparfile,t0,tstop,dt,dtout,dtdump,
     &     iflgchk,rmin,rmax,rmaxu,qmin,lclose,outfile,fopenstat)

c Get parameters used in treecode
      write(*,*) 'Enter name of treecode parameter data file :'
      read(*,999) intreefile
      call symbatr_treeparam(intreefile)
      print *,'after back from symbatr_treeparam,'
      print *,'treefreq, eps,theta =',treefreq,eps,theta
...
c Prompt and read name of planet data file
 ...
 999  format(a)
     ...
     &     xh,yh,zh,vxh,vyh,vzh,rpl,rhill,j2rp2,j4rp4)
      ...
      write(*,*) 'Enter mass limit below which treecode is used'
      read(*,*) mtiny
      write(*,*) ' mtiny = ',mtiny
...
c...  Calculate the location of the last massive particle
      call symbatr_nbodm(nbod,mass,mtiny,nbodm)
...
c     TEST:
c      open(unit=50, name='taccel1.out')
c      open(unit=51, name='taccel10.out')
c      open(unit=52, name='taccel50.out')
c      open(unit=53,name='taccel100.out')
c      open(unit=54,name='taccel500.out')
c***************here's the big loop *************************************
      ...
      do while ( (t .le. tstop) .and. (nbod.gt.1) )
         ...
         call symbatr_step_pl(i1st,t,nbod,nbodm,mass,j2rp2,j4rp4,
