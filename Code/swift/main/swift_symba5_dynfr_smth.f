...
c     morby's definitions
      real*8 dEa(NSTEP,NBFRIC),dGa(NSTEP,NBFRIC),dHa(NSTEP,NBFRIC)
      integer istp,isub,j
      real*8 a11,a12,a13,a21,a22,a23,a31,a32,a33
      real*8 b11,b12,b13,b21,b22,b23,b31,b32,b33
      real*8 Gx,Gy,Gz,G,detA
      common /fric/dEa,dGa,dHa
c-----
c...  Executable code 
...
c...  Calculate the location of the last massive particle
      ...
      if(nbodm.gt.NBFRIC)then
         write(*,*)'parameter NBFRIC is not large enough'
         stop
      endif

c     initialize to zero the dEa, dGa, dHa arrays...
      do i=1,NBFRIC
         do istp=1,NSTEP
            dEa(istp,i)=0.d0
            dGa(istp,i)=0.d0
...
c     comupute the current step and substep of the dE,dG,DH arrays
         isub=isub+1
         if(isub.gt.NSUBSTEP)then
            isub=isub-NSUBSTEP
            do j=2,nbodm
               dEa(istp,j)=0.d0
               dGa(istp,j)=0.d0
            ...
            enddo
            istp=istp+1
            if(istp.gt.NSTEP)istp=istp-NSTEP
         endif
c     on each big body apply dynamical friction
         do j=2,nbodm         
c (dE,dG,dH)=A*(dvx,dvy,dvz): computation of the coefficients of the matrix A
            Gx=(yh(j)*vzh(j) - zh(j)*vyh(j))
            Gy=(zh(j)*vxh(j) - xh(j)*vzh(j))
            Gz=(xh(j)*vyh(j) - yh(j)*vxh(j))
            G=sqrt(Gx**2+Gy**2+Gz**2)
            a11=vxh(j)
            a12=vyh(j)
            a13=vzh(j)
            a21=(Gy*zh(j)-Gz*yh(j))/G
            a22=(Gz*xh(j)-Gx*zh(j))/G
            a23=(Gx*yh(j)-Gy*xh(j))/G
            a31=-yh(j)
            a32=xh(j)
c     calculation of the coefficients of B=A^{-1}
            detA=(a11*a23-a13*a21)*a32-a31*(a12*a23-a13*a22)
            b11=a23*a32/detA
            b12=-a13*a32/detA
            b13=-(a12*a23-a13*a22)/detA
            b21=-a23*a31/detA
            b22=a13*a31/detA
            b23=(a11*a23-a13*a21)/detA
            b31=(-a21*a32+a22*a31)/detA
            b32=(a11*a32-a12*a31)/detA
            b33=-(a11*a22-a12*a21)/detA
c   now compute (dvx,dvy,dvz)=B(dE,dG,dH)
           vxh(j)=vxh(j)+b11*dEa(istp,j)+b12*dGa(istp,j)+b13*dHa(istp,j)
           vyh(j)=vyh(j)+b21*dEa(istp,j)+b22*dGa(istp,j)+b23*dHa(istp,j)
           vzh(j)=vzh(j)+b31*dEa(istp,j)+b32*dGa(istp,j)+b33*dHa(istp,j)
        enddo
...
c...  Internals:
      ...
      real*8 s,b,dU,U,bhill
      logical*1 svdotr(NENMAX)  ! Used by symba_step_recur
      real*8 dEa(NSTEP,NBFRIC),dGa(NSTEP,NBFRIC),dHa(NSTEP,NBFRIC)
      integer istp
      real*8 Gx,Gy,Gz,G,dvx,dvy,dvz,dG,dE,dH
...
c     now apply a velocity change dU to the massive guy j, directed as (vxr,vyr,vzr)
               ...
               Gz=(xh(j)*vyh(j) - yh(j)*vxh(j))
               G=sqrt(Gx**2+Gy**2+Gz**2)
               dE=vxh(j)*dvx + vyh(j)*dvy + vzh(j)*dvz
               dG=((Gy*zh(j)-Gz*yh(j))*dvx+
     $              (Gz*xh(j)-Gx*zh(j))*dvy+
     $              (Gx*yh(j)-Gy*xh(j))*dvz)/G
               dH=-yh(j)*dvx+xh(j)*dvy
               do istp=1,NSTEP
                  dEa(istp,j)=dEa(istp,j)+dE/NSTEP/NSUBSTEP
                  dGa(istp,j)=dGa(istp,j)+dG/NSTEP/NSUBSTEP
                  dHa(istp,j)=dHa(istp,j)+dH/NSTEP/NSUBSTEP
               enddo
            endif
         endif
      enddo
