c**********************************************************************
c		      SWIFT_RMVS3_CORR.F
c**********************************************************************
c
c                 INCLUDES CLOSE ENCOUNTERS & CORRECTOR STEPS
...
c Date:    Feb 14, 2023
c Remark : Adapted from swift_rmvs3.f
	...
	integer istat(NTPMAX,NSTAT),i1st,nsta
	...
	character*(chlen) outfile,inparfile,inplfile,intpfile,
     &       fopenstat,diro,dirs,gname,genfile,dataname
        integer*8 iloop,noutloop,ndumploop,ialpha,i
	real*8 a,e,q
...
c***************here's the big loop *************************************
     ...
     &       ((ntp.eq.0).or.(nleft.gt.0)) )
 	     ...
 	     call rmvs3_step_corr(nsta,i1st,t,nbod,ntp,mass,
     &	           j2rp2,j4rp4,xh,yh,zh,vxh,vyh,vzh,xht,yht,zht,
     &	           vxht,vyht,vzht,istat,rstat,dt)

c	     t = t + dt

	     iloop = iloop+1_8*nsta
             t = t0 + dble(iloop)*dt

c             do i=2,nbod
c		call orbel_xv2aeq(xh(i),yh(i),zh(i),
c     &		vxh(i),vyh(i),vzh(i),mass(1),ialpha,a,e,q)
c                write(*,*)'planet',i,'t=',sngl(t),
c     &             		'a,e= ',sngl(a),sngl(e)
c             end do
...
c          if(t.ge.tdump) then
 ...
 998         format(' Time = ',1p1e12.5,': fraction done = ',0pf5.3,
     ...
     &           xht,yht,zht,vxht,vyht,vzht,istat,rstat)
	     call io_dump_param_corr(trim(diro)//'/'//'dump_param.dat',
     &          t,tstop,dt,dtout,dtdump,iflgchk,nsta,rmin,rmax,rmaxu,
