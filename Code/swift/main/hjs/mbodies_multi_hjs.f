...
C
     ...
     &                   TEPX,TEPY,TEPZ
        ...
        real*8 xjt(10*NTPMAX),yjt(10*NTPMAX),zjt(10*NTPMAX)
        real*8 vxjt(10*NTPMAX),vyjt(10*NTPMAX),vzjt(10*NTPMAX)
        real*8 matp(NPLMAX,10*NTPMAX),umatp(NPLMAX,10*NTPMAX)
        REAL*8 MATPNS(NPLMAX,NTPMAX),UMATPNS(NPLMAX,NTPMAX)
        ...
        REAL*8 ETATPNS(NTPMAX)
        REAL*8 DTSNAP,RR,TIM,XT,YT,ZT,VXT,VYT,VZT,TDEB,q
        INTEGER NBOD,NTP,IERR,IFOL,ISTEP,KBIN,REFNS
        INTEGER OLOCTNS(NPLMAX,NTPMAX),OLOCT(NPLMAX,10*NTPMAX)
        INTEGER IFLGCHK,IU,NLEFT,I,ID,J,ISIZ,ISNAPST,SNAPST
     ...
     &         gname(i),outfile(i),fopenstat)
        ...
        CALL IO_INIT_PL_HJS(INPLFILE,LCLOSE,IFLGCHK,NBOD0,OLOC,
     &       MASS,ETA,MU,MAT,UMAT,XJ,YJ,ZJ,VXJ,VYJ,VZJ,RPLSQ)
        ...
        DO NS = 1,NBSIMU
          ...
          read(*,'(a)') intpfile(ns)
          call io_init_tp_hjs(intpfile(ns),nbod0,ntp0(ns),oloc,oloctns,
     &           mass,umat,etatpns,matpns,umatpns,xjt,yjt,zjt,
     &           vxjt,vyjt,vzjt,istat,rstat)
          j = sum(ntp0(1:ns-1))
          MATP(:,j+1:j+NTP0(NS)) = MATPNS(:,1:NTP0(NS))
          UMATP(:,j+1:j+NTP0(NS)) = UMATPNS(:,1:NTP0(NS))
          ETATP(j+1:j+NTP0(NS)) = ETATPNS(1:NTP0(NS))
          OLOCT(:,j+1:j+NTP0(NS)) = OLOCTNS(:,1:NTP0(NS))
          do i=1,ntp0(ns)
            call orbel_xv2aeq(xjt(i),yjt(i),zjt(i),
     &        vxjt(i),vyjt(i),vzjt(i),etatp(i+j),ialpha,ainit(j+i),e,q)
          end do
        END DO
...
c
     ...
     &    R,XJ(-ID),YJ(-ID),ZJ(-ID),VXJ(-ID),VYJ(-ID),VZJ(-ID),
...
c     &          (CAPMB(-ID)-CAPMB0(-ID))   ! *180.d0/PI
     ...
     &          XJ(1:NBOD),YJ(1:NBOD),ZJ(1:NBOD),VXJ(1:NBOD),
     &          VYJ(1:NBOD),VZJ(1:NBOD),XB(1:NBOD),YB(1:NBOD),
     &          ZB(1:NBOD),VXB(1:NBOD),VYB(1:NBOD),VZB(1:NBOD))
              DO I=1,NBOD
...
c-------------------------------------------------------------------------
     ...
     &     TCAPOM,TOMEGA,TCAPM
      ...
      READ(*,*)NCG
      WRITE(*,*) ' '
      call io_init_pl_hjs(inplfile,lclose,iflgchk,nbod,oloc,mass,
     &     eta,mu,mat,umat,xj,yj,zj,vxj,vyj,vzj,rplsq)
      IU = 20
      NB = 0
      NBC = 0
      DR = 180.0D0/PI
