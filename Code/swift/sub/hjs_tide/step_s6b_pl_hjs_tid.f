...
c K(dt1).T(dt1).D(dt2).K(dt3).T(dt3).D(dt4).T(dt3).K(dt3).D(dt2).T(dt1).K(dt1)
c
c             Input:
...
c                                       (real arrays)         
c
c Remarks: Adapted from step_kdk_pl_hjs_tid.f & step_s6b_pl_hjs.f
c Authors:  Herve Beust
c Date:    Mar 25, 2010

      subroutine step_s6b_pl_hjs_tid(i1st,nbod,tid,oloc,umat,mat,mass,
...
c...  substep T(dt1) : Do the tides for dt1
      call kickvj_tid(nbod,tid,oloc,mass,inert,krpl5,Qlov,
     &     eta,mu,xj,yj,zj,vxj,vyj,vzj,dt1,rotx,roty,rotz)
c...  substep D(dt2) : First Drift in Jacobi coords for dt2
      call drift_hjs(nbod,mass,eta,mu,xj,yj,zj,vxj,vyj,vzj,dt2)

c...  substep K(dt3) :compute bary. xb and vb for acceleration calculations
      call coord_g2b(nbod,umat,mass,xj,yj,zj,vxj,vyj,vzj,
...
c...  substep T(dt3) : Do the tides for dt3
      call kickvj_tid(nbod,tid,oloc,mass,inert,krpl5,Qlov,
     &     eta,mu,xj,yj,zj,vxj,vyj,vzj,dt3,rotx,roty,rotz)

c...  substep D(dt4) : Second Drift in Jacobi coords for dt4
...
c...  substep D(dt2) : Third Drift in Jacobi coords for dt2
      call drift_hjs(nbod,mass,eta,mu,xj,yj,zj,vxj,vyj,vzj,dt2)
...
c...  substep T(dt1) : Do the tides for dt1
      call kickvj_tid(nbod,tid,oloc,mass,inert,krpl5,Qlov,
     &     eta,mu,xj,yj,zj,vxj,vyj,vzj,dt1,rotx,roty,rotz)
c...  substep K(dt1) : Compute bary. xb and vb for acceleration calculations
      call coord_g2b(nbod,umat,mass,xj,yj,zj,vxj,vyj,vzj,
