...
c          ==> Takes tide and relativity into account
c
c             Input:
...
c
c
c Remarks: Adopted from step_dkd_corr_hjs.f
c Authors:  Herve Beust 
c Date:    Mar 11, 2010

      subroutine step_dkd_corr_hjs_tid(nsta,s,i1st,time,nbod,tid,ntp,
...
c...  Do the nsta stages of correction
      ...
      do i = 1,nsta
        call step_dkd_corr_z_hjs_tid(aa(i),bb(i),i1st,time,nbod,
     &     tid,ntp,oloc,oloct,mat,umat,matp,umatp,mass,inert,
     &     krpl5,Qlov,eta,mu,etatp,xj,yj,zj,vxj,vyj,vzj,
     &     rotx,roty,rotz,xjt,yjt,zjt,vxjt,vyjt,vzjt,istat,rstat,dt)	
      end do

      return
      end   ! step_dkd_corr_hjs_tid
c------------------------------------------------------------------------
