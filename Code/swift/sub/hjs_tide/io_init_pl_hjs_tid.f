c************************************************************************
c                              IO_INIT_PL_HJS_TID.F
c************************************************************************
c IO_INIT_PL_HJS_TID reads in the data for the massive bodies for HJS 
c                                 with tides
c             Input:
...
c                                    (real array)
c
c Remarks: Adapted from io_init_pl_hjs.f
c Authors:  Herve Beust
c Date:    Jun 6, 2008; revised Jun 19

	subroutine io_init_pl_hjs_tid(infile,lclose,iflgchk,nbod,
     &     tid,oloc,mass,eta,mu,mat,umat,xj,yj,zj,vxj,vyj,vzj,
...
c...   Internal
	integer j,i,ierr
        real*8 rpl,mtot,vsat,vcen,klov,rgyr
...
c... Read information relative to bodies: masses, bary pos & vels.
	do j=1,nbod
           if(lclose) then
              read(7,*) mass(j),rpl,klov,Qlov(j),rgyr
              write(*,*) mass(j),rpl
              rplsq(j) = rpl*rpl
              inert(j) = rgyr*rgyr*mass(j)*rplsq(j)
              krpl5(j) = klov*rplsq(j)*rplsq(j)*rpl
           else
              read(7,*) mass(j)
              write(*,*) mass(j)
           endif
