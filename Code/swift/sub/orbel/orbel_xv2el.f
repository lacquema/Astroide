...
*     REVISIONS: H. Beust, Nov 4, 1998.
*                ==> add hyperbolic repulsive case for particles
*                    having a radiation pressure larger than 
*                    solar gravity
*                H. Beust & R. Reche, May 31, 2005
*                ==> Fixed bug in the computation of u at low eccentricity
***********************************************************************
...
c...  Internals:
	...
	real*8 cw,sw,w,u,scapom,ccapom
...
* latitude u. (u = arg of periastron + true anomaly)
	...
	if(fac.lt. TINY ) then
	  ...
	  capom = atan2(hx,-hy)	  
          ccapom = cos(capom)
          scapom = sin(capom)
          if ((inc.lt.0.5d0) .or. (inc.gt. (PI-0.5d0))) then 
            u = atan2((-x*scapom + y*ccapom)/cos(inc),
     &                                x*ccapom + y*scapom)
          else  
    	    u = atan2 ( z/sin(inc) , x*ccapom + y*scapom)
          end if
	endif
...
***
***
c HYPERBOLA (repulsive case. Valid when beta(rad. press)>1, ie gmsum<0)
	if(ialpha .eq. +2) then
	  a = -0.5d0*gmsum/energy  
	  fac = h2/(gmsum*a)
