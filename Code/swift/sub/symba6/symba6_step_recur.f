...
c Last revision: 5/13/99
      ...
      recursive subroutine symba6_step_recur(t,nbod,nbodm,mass,
...
c...     Do we need to go deeper?
         ...
         do ie=1,ielc
            ...
            if((ielev(i).ge.ireci).and.(ielev(j).ge.ireci)) then
               call symba6_chk(rhill,nbod,i,j,mass,xh,yh,zh,
     &              vxb,vyb,vzb,dtl,irecp,ieflg,svdotr(ie))
         ...
         sgn = 1.0d0
         call symba6_kick(nbod,mass,irecp,iecnt,ielev,
     &        rhill,xh,yh,zh,vxb,vyb,vzb,dth,sgn,ielc,
     &        ielst)

         call symba6_helio_drift(nbod,ielev,ireci,mass,xh,
     &        yh,zh,vxb,vyb,vzb,dtl)
         if(icflg.ne.0) then
            call symba6_step_recur(t,nbod,nbodm,mass,irecp,ilevl,
     ...
     &        rhill,xh,yh,zh,vxb,vyb,vzb,dth,sgn,ielc,ielst)
         ...
         if( lclose ) then       ! look for mergers
            do ie=1,ielc
               ...
               j = ielst(2,ie)
               if((ielev(i).ge.ireci).and.(ielev(j).ge.ireci)) then
                  call symba6_merge(t,dtl,nbod,i,j,mass,xh,yh,zh,
...
c...        Do we need to go deeper?
            ...
            do ie=1,ielc
               ...
               j = ielst(2,ie)
               if((ielev(i).ge.ireci).and.(ielev(j).ge.ireci)) then
                  call symba6_chk(rhill,nbod,i,j,mass,xh,yh,
